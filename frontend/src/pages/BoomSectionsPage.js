...
const BoomSectionForm = ({ initial, onSave, onClose, endpoint }) => {
  const [form, setForm] = useState({
    ...
    weightKg: '', location: '', condition: 'Ok', status: 'Available', comments: ''
  });
  const [saving, setSaving] = useState(false);
  const [step, setStep] = useState(1);
  const steps = [
    { title: 'Basic details', description: 'Enter the boom section identity and dimensions.' },
    { title: 'Assignment', description: 'Link it to a crane and mark its condition.' },
    ...
    <form onSubmit={handleSubmit} style={{ display: 'grid', gap: '14px' }}>
      ...
      {step === 1 && (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            <label className="label">Boom Code</label>
            <input type="text" className="input-field" value={form.boomCode || ''} onChange={e => handleChange('boomCode', e.target.value)} />
          </div>
          <div>
          ...
          <div>
            ...
            <input type="text" className="input-field" value={form.length || ''} onChange={e => handleChange('length', e.target.value)} />
          </div>
          <div>
      ...
      {step === 2 && (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div style={{ position: 'relative' }}>
            ...
            <p style={{ margin: '5px 0 0', fontSize: '11px', color: 'var(--text-muted)' }}>Leave blank to make this boom section available to all cranes.</p>
        ...
        <div style={{ display: 'grid', gap: '12px' }}>
          ...
          <div className="wizard-section-card wizard-summary-card">
            ...
            <div style={{ display: 'grid', gap: '6px', fontSize: '13px', color: 'var(--text-primary)' }}>
              <div><strong>Boom code:</strong> {form.boomCode || '—'}</div>
...
export default createEquipmentPage({
  ...
  columns: [
    ...
    { key: 'craneModel', label: 'Model' },
    { key: 'location', label: 'Location' },
    { key: 'condition', label: 'Condition', badge: true },
    { key: 'status', label: 'Status', badge: true },
  ],
  FormComponent: BoomSectionForm,
});
