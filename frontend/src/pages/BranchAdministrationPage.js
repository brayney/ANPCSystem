...
export default function BranchAdministrationPage() {
  ...
  const [form, setForm] = useState({ branchName: '', branchCode: '', branchAddress: '', name: '', email: '', password: '' });
  const [edit, setEdit] = useState({ name: '', code: '', address: '' });
  ...
  const openReviewBranch = () => {
    setActiveView('review');
    setSelected(null);
    setDetails(null);
    setTimeout(() => document.getElementById('branch-list-card')?.scrollIntoView({ behavior: 'smooth', block: 'start' }), 0);
  };
  ...
  const review = async (branch) => {
    setActiveView('review');
    setSelected(branch);
    setEdit({ name: branch.name, code: branch.code, address: branch.address || '' });
    setDetails(null);
  ...
  const submit = async (event) => {
    ...
    try {
      ...
      setForm({ branchName: '', branchCode: '', branchAddress: '', name: '', email: '', password: '' });
      load();
    } catch (error) {
  ...
  const toggle = async (branch) => {
    try {
      ...
      if (selected?._id === branch._id) {
        review({ ...branch, isActive: !branch.isActive });
      }
    } catch (error) {
      toast.error(error.response?.data?.message || 'Unable to update branch');
    }
    ...
    if (activeView === 'add') {
      return (
        <div style={{ display: 'grid', gap: 16 }}>
          <button type="button" className="btn-secondary" style={{ justifySelf: 'start' }} onClick={goBackToCards}>← Back</button>
          ...
          <div>
            ...
            <p style={{ margin: '4px 0 0', color: 'var(--text-secondary)', fontSize: 13 }}>New branches begin with no operational data.</p>
          </div>
          {[['Branch name', 'branchName', 'text'], ['Branch code', 'branchCode', 'text'], ['Branch address', 'branchAddress', 'text'], ['Administrator name', 'name', 'text'], ['Administrator email', 'email', 'email'], ['Temporary password', 'password', 'password']].map(([label, key, type]) => (
            <div key={key}>
              <label className="label">{label}{key !== 'branchAddress' && ' *'}</label>
              <input required={key !== 'branchAddress'} className="input-field" type={type} value={form[key]} onChange={e => setForm({ ...form, [key]: e.target.value })} />
            </div>
          ))}
        ...
        {!selected ? (
          <div style={{ display: 'grid', gap: 20, width: '100%' }}>
            ...
            <div id="branch-list-card" className="card">
              <h2 style={{ marginTop: 0, fontSize: 16 }}>Branches</h2>
                ...
                <div style={{ display: 'grid', gap: 10 }}>
                  {branches.map(branch => (
                    <div key={branch._id} className="branch-admin-card-item">
                      <button type="button" className="branch-admin-card-button" onClick={() => review(branch)}>
          ...
          <div className="card" style={{ width: '100%', padding: '24px', borderRadius: '18px' }}>
            {loadingDetails ? <Spinner size="sm" /> : details && (
              <>
                ...
                <form onSubmit={saveEdit} style={{ display: 'grid', gridTemplateColumns: 'repeat(3, minmax(180px, 1fr)) auto', gap: 12, alignItems: 'end', marginBottom: 24 }}>
                  <div>
                  ...
                  <div>
                    ...
                    <input className="input-field" value={edit.address} onChange={e => setEdit({ ...edit, address: e.target.value })} />
                  </div>
  ...
  return (
    <div className="page-container branch-admin-page">
      <header className="company-admin-topbar">
        <div className="company-admin-topbar-brand">
          <img src="/logo.png" alt="ANPC Logo" className="company-admin-topbar-logo" />
          <div>
            <h1>Branch Administration</h1>
            <p>Create, review, and manage independent branch workspaces.</p>
          </div>
        </div>
        ...
        <div className="company-admin-topbar-actions" data-company-admin-profile-root>
          ...
          <div className="company-admin-profile-menu-wrap">
            <button type="button" className="company-admin-profile-trigger" onClick={() => setProfileMenuOpen(prev => !prev)}>
              <div className="company-admin-profile-avatar">{user?.name?.[0]?.toUpperCase() || 'A'}</div>
              <div className="company-admin-profile-copy">
                <strong>{user?.name || 'Admin'}</strong>
                <span>{user?.email || 'Company admin'}</span>
              </div>
            </button>
      ...
      {!activeView && (
        <div className="branch-admin-action-cards">
          <button type="button" className="branch-admin-action-card branch-admin-action-card-primary" onClick={openAddBranch}>
            <span className="branch-admin-action-card-kicker">Quick action</span>
            <strong>Add branch</strong>
            <span>Create a new branch workspace and administrator account.</span>
          </button>
          <button type="button" className="branch-admin-action-card" onClick={openReviewBranch}>
            <span className="branch-admin-action-card-kicker">Review</span>
            <strong>Review branch</strong>
            <span>Open a branch summary and manage its status.</span>
          </button>
        </div>
      )}

      <div className="branch-admin-grid" style={{ display: 'grid', gridTemplateColumns: 'minmax(280px, 390px) minmax(0, 1fr)', gap: 20, alignItems: 'start' }}>
        {renderContent()}
      </div>
    </div>
  );
